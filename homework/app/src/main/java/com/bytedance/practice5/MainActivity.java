...
public class MainActivity extends AppCompatActivity {
    ...
    private void getData(String studentId){
        new Thread(new Runnable() {
            ...
            public void run() {
                Log.d(TAG,"run");
                List<Message> messages = baseGetReposFromRemote(
                        studentId, "WkpVLWJ5dGVkYW5jZS1hbmRyb2lk");
    ...
    public List<Message> baseGetReposFromRemote(String studentId, String accept) {

        String urlStr =
                String.format("https://api-android-camp.bytedance.com/zju/invoke/messages?%s", studentId);
        List<Message> result = null;
        MessageListResponse  temp = null;
...
//            conn.setRequestProperty("accept", accept);
            ...
            if (conn.getResponseCode() == 200) {
                ...
                temp = new Gson().fromJson(reader, new TypeToken<MessageListResponse>() {
                }.getType());
                result = temp.feeds;
                reader.close();
                in.close();
